...
def pretrain(args):
    ...
    if args.opticaldir is not None:
        ...
        opticalpaths.sort()
        opticalprocpaths = [os.path.join(args.opticalprocdir, os.path.basename(opticalpath)) for opticalpath in
                            opticalpaths]
    else:
...
#            out_file=maskpath,
#        )
        maskdata = sol.vector.mask.df_to_px_mask(
            df=gdf,
...
def test(args):
    ...
    folders = [args.testbinarydir, args.testvectordir]
    for folder in folders:
        makeemptyfolder(folder)
